...
std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
  ...
  encfile.open(argv[2], std::ios::binary | std::ios::trunc);
  ...
  for (size_t i = 0; i < (results.st_size / 4); i++) {
    if(!inputfile.read((char*)bytevals, 4)) {
    ...
    if(!encfile.write((char*)bytevals255, 5)) {
  ...
  if(!inputfile.read((char*)remaining, remaint)) {
    std::cout << "Failed to read final iteration" << '\n';
  }
  for (size_t i = 0; i < remaint; i++) {
    remaining[i] ^= 0xBE;
  }
  if(!encfile.write((char*)remaining, remaint)) {
